...
class AdvLitModule(LightningModule):
    ...
    def __init__(
        ...
        blackbox_ = [model for model in self.attack_model_name_list if model not in self.train_model_name_list]
        assert self.blackbox == blackbox_[0]

        # 读取loss_img
        self.loss_img_list = []
        for img_name in sorted(os.listdir(self.loss_img_root)):
            if img_name in self.loss_name_list:
                loss_img = self.read_img(os.path.join(self.loss_img_root, img_name), 0.5, 0.5, self.device)
                self.loss_img_list.append(loss_img)
        ...
        self.val_inter_log_path = os.path.join(self.val.dir, self.val.name.replace('val', 'val_inter'))


        #TODO input size (128, 128)
    ...
    def preprocess(self, im, mean, std, device):
        if len(im.size()) == 3:
    ...
    def read_img(self, data_dir, mean, std, device):
        img = cv2.imread(data_dir)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) / 255
        img = torch.from_numpy(img).to(torch.float32).to(device)
        img = self.preprocess(img, mean, std, device)
        return img
    ...
    def on_train_start(self):
        ...
        if self.loss_img_list[0].device != self.device:
            for idx, _ in enumerate(self.loss_img_list):
                self.loss_img_list[idx] = self.loss_img_list[idx].to(self.device)
    ...
    def full_loss(self, detect_ganis, shapes, dts, reals, fakes): 
        ...
        for b_idx in range(bz):
            ...
            if self.g_dt_loss_mode == "perception":
                ...
                g_loss_detect = F.mse_loss(fakes[b_idx, :, crop_start_y:crop_end_y, crop_start_x:crop_end_x], detect_ganis_resize) * self.g_p_dt_w
            ...
            if self.g_bg_loss_mode == "perception":
                ...
                g_loss_bg = F.mse_loss(fakes[b_idx,:] * mask, reals[b_idx,:] * mask) * self.g_p_bg_w
    ...
    def train_generator(self):
        ...
        if self.g_part_loss_mode == "perception":
            ...
            g_loss_eye = F.mse_loss(x_fake_eye, x_pre_eye) * self.g_p_part_w
        else:
            raise RuntimeError("no valid loss mode")
        ...
        if self.g_part_loss_mode == "perception":
            ...
            g_loss_nose = F.mse_loss(x_fake_nose, x_pre_nose) * self.g_p_part_w 
        else:
            raise RuntimeError("no valid loss mode")
        ...
        if self.g_part_loss_mode == "perception":
            ...
            g_loss_mouth = F.mse_loss(x_fake_mouth, x_pre_mouth) * self.g_p_part_w 
        else:
            raise RuntimeError("no valid loss mode")
        ...
        for target_image in self.iter_loss_img_list: 
            for model_name in self.train_model_name_list:
                for i in range(self.diversity):
                    target_loss_x = self.cal_adv_loss(fake_x_diversity[i], target_image.repeat(x_fake.shape[0],1,1,1), model_name, self.targe_models) * self.lambda_adv * 0.5
        ...
        if self.local_step % self.train_freq == 0:
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        self.iter_loss_img_list = random.sample(self.loss_img_list, 1)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        self.log("current_epoch", self.current_epoch, on_step=False, on_epoch=True)
        if self.current_epoch == 15:
            self.trainer.callbacks[-1]._every_n_epochs = 1 
        pass
